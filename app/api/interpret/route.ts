...
export async function POST(request: NextRequest) {
  try {
    ...
    const { chart, messages } = body as { chart: ZiweiChart; messages: { role: 'user' | 'assistant'; content: string }[] };
    if (!chart || !messages || !Array.isArray(messages)) {
      return NextResponse.json({ error: '缺少必填字段：chart, messages' }, { status: 400 });
    }
    ...
    if (userId) {
      ...
      if (!isPro) {
        ...
        if (count >= 3) {
          return NextResponse.json({ error: '免费用户每天限 3 次解读，升级会员解锁无限次' }, { status: 429 });
        }
      }
    }
