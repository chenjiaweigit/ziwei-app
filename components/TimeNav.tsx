  ...
  return (
    <div className="mb-3">
      <div className="flex items-center rounded-xl p-1 gap-1" style={{ background: 'var(--t-surface)', border: '1px solid var(--t-border)' }}>
        <TabButton active={view === 'mingpan'} onClick={() => onViewChange('mingpan')}>本命</TabButton>
        <TabButton active={view === 'daxian'} onClick={() => onViewChange('daxian')}>{currentDx ? `大限 ${currentDx.startAge}–${currentDx.endAge}` : '大限'}</TabButton>
        <div className="relative flex-1 flex items-center justify-center rounded-lg py-1.5 gap-1 transition-all duration-200"
          style={{ background: view === 'liunian' ? 'rgba(212,168,67,0.12)' : 'transparent', border: view === 'liunian' ? '1px solid rgba(212,168,67,0.25)' : '1px solid transparent' }}>
          <button onClick={() => onViewChange('liunian')} className="text-[10px] font-medium flex-1 text-center"
            style={{ color: view === 'liunian' ? 'var(--t-gold)' : 'var(--t-faint)' }}>流年</button>
          <div className="flex items-center gap-0.5">
            ...
            <span className="text-[10px] font-mono min-w-[28px] text-center cursor-pointer"
      ...
      {overlayInfo && (
        <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}
          ...
          <span className="text-[9px]" style={{ color: 'var(--t-faint)' }}>{view === 'daxian' ? '大限' : `${liunianYear}`}·{overlayInfo.stemName}年四化：</span>
...
function TabButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick}
      ...
      style={{ background: active ? 'rgba(212,168,67,0.12)' : 'transparent', color: active ? 'var(--t-gold)' : 'var(--t-faint)', border: active ? '1px solid rgba(212,168,67,0.25)' : '1px solid transparent' }}>
      {children}
    </button>
  );
}