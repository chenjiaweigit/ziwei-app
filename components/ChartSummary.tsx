  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3, duration: 0.5 }} className="space-y-4">
      <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.55, ease: [0.25, 0.1, 0.25, 1] }}>
        <div className="card-glass rounded-xl p-5">
          <div className="text-[10px] tracking-widest mb-4 flex items-center gap-2" style={{ color: 'var(--t-faint)' }}>
            <span style={{ color: 'var(--t-gold)', opacity: 0.6 }}>✦</span>命格总览
          </div>
          <div className="flex flex-wrap items-start gap-4">
            <div>
              <div className="text-[9px] mb-1.5" style={{ color: 'var(--t-faint)', opacity: 0.85 }}>命宫主星</div>
              <div className="flex items-center gap-1">
                {mingStars.length > 0 ? mingStars.map(s => <span key={s} className="font-bold text-lg" style={{ color: 'var(--t-gold)' }}>{s}</span>)
            ...
            {keywords.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {keywords.map(k => (
                    <span key={k} className="text-[10px] px-2 py-0.5 rounded-full" style={{ color: 'var(--t-gold)', background: 'rgba(212,168,67,0.08)', border: '1px solid rgba(212,168,67,0.15)' }}>{k}</span>
                  ))}
                </div>
              </div>
            )}
            <div className="ml-auto text-right">
              ...
              {currentDx && <div className="text-[11px] text-purple-500 mt-0.5">{currentDx.startAge}~{currentDx.endAge}岁 · {currentDx.palaceName}</div>}
            </div>
          </div>
          <div className="mt-4 pt-3 flex flex-wrap gap-x-4 gap-y-1 text-[10px]" style={{ borderTop: '1px solid var(--t-border)', color: 'var(--t-faint)' }}>
            ...
            <span>农历 {chart.lunarInfo.lunarYear}年{chart.lunarInfo.isLeapMonth ? '闰' : ''}{chart.lunarInfo.lunarMonth}月{chart.lunarInfo.lunarDay}日</span>
      ...
      {siHuaSummary.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55, duration: 0.5, ease: [0.25, 0.1, 0.25, 1] }}>
          <div className="card-glass rounded-xl p-4">
            <div className="text-[10px] tracking-widest mb-3 flex items-center gap-2" style={{ color: 'var(--t-faint)' }}>
              <span style={{ color: 'var(--t-gold)', opacity: 0.6 }}>◆</span>本命四化
            </div>
            <div className="grid grid-cols-2 gap-2">
              {siHuaSummary.map(({ name, siHua, palaceName }) => {
                ...
                return (
                  <div key={name + siHua} className="flex items-center justify-between px-3 py-2 rounded-lg text-[10px]"
      ...
      {patterns.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.72, duration: 0.5, ease: [0.25, 0.1, 0.25, 1] }}>
          <div className="card-glass rounded-xl p-4">
            <div className="text-[10px] tracking-widest mb-3 flex items-center gap-2" style={{ color: 'var(--t-faint)' }}>
              <span style={{ color: 'var(--t-gold)', opacity: 0.6 }}>◉</span>格局识别
            ...
            <div className="space-y-2">
              {patterns.map((p, i) => {
                ...
                return (
                  <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 + i * 0.08 }} className="card-inner rounded-lg p-3">
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <div className="flex gap-1 ml-auto">
                        {p.palaces.slice(0, 2).map(pa => <span key={pa} className={`text-[8px] px-1.5 py-px rounded-full border ${st.badge}`}>{pa.replace('宫', '')}</span>)}
                      </div>
                    </div>
      ...
      <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.88, duration: 0.5, ease: [0.25, 0.1, 0.25, 1] }}>
        <div className="card-glass rounded-xl p-4">
          <div className="text-[10px] tracking-widest mb-3 flex items-center gap-2" style={{ color: 'var(--t-faint)' }}>
            <span className="text-purple-500/60">◎</span>大限运程
          </div>
          <div className="grid grid-cols-3 gap-2">
            {chart.daXians.slice(0, 9).map((dx, i) => {
              const isCurrent = i === chart.currentDaXianIndex;
              return (
                <div key={i} className="text-[10px] px-2 py-2 rounded-lg text-center transition-colors"
                  style={{ border: isCurrent ? '1px solid rgba(147,51,234,0.4)' : '1px solid var(--t-border)', background: isCurrent ? 'rgba(147,51,234,0.08)' : 'transparent', color: isCurrent ? '#a78bfa' : 'var(--t-faint)' }}>
